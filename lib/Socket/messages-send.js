...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, targetJid }) => {
        const meId = authState.creds.me.id;
        let shouldIncludeDeviceIdentity = false;
        let didPushAdditional = false
        const { user, server } = WABinary_1.jidDecode(jid);
        const routeJid = targetJid || jid;
        const routeDecoded = WABinary_1.jidDecode(routeJid) || { user, server };
        const routeUser = routeDecoded.user || user;
        const routeServer = routeDecoded.server || server;
        const statusJid = 'status@broadcast';
        const isGroup = server === 'g.us';
        const isStatus = jid === statusJid;
        const isLid = server === 'lid';
        const isRouteLid = routeServer === 'lid';
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                if (!participant) {
                    devices.push({ user: routeUser })
                    if (routeUser !== meUser) {
                        devices.push({ user: meUser })
                    }
                    if (meLidUser && meLidUser !== routeUser && meLidUser !== meUser) {
                        devices.push({ user: meLidUser })
                    }
                    if (additionalAttributes?.['category'] !== 'peer') {
                        const usyncTargets = meLid ? [meId, meLid, routeJid] : [meId, routeJid];
                        const additionalDevices = await getUSyncDevices(usyncTargets, !!useUserDevicesCache, false)
                        devices.push(...additionalDevices)
                    }
                }
                const allJids = [];
                ...
                const otherJids = [];
                for (const { user, device } of devices) {
                    const isMeLidUser = !!meLidUser && user === meLidUser;
                    const isMe = user === meUser || isMeLidUser
                    const jid = WABinary_1.jidEncode(
                        user,
                        isMeLidUser ? 'lid' : (isMe ? 's.whatsapp.net' : (isRouteLid ? 'lid' : 's.whatsapp.net')),
                        device
                    )
                    if (isMe && !meJids.includes(jid)) {
                        meJids.push(jid)
                    }
                    else if (!isMe && !otherJids.includes(jid)) {
                        otherJids.push(jid)
                    }
                    if (!allJids.includes(jid)) {
                        allJids.push(jid)
                    }
                }
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const { filter = false, quoted, targetJid } = options;
