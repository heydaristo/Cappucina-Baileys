...
async function convertToOpusBuffer(buffer, logger) {
    // Use file-based conversion to avoid stream race conditions
    // that cause truncated/invalid OGG output on iOS
    ...
    const inputPath = path.join(tmpDir, `opus_in_${uid}`);
    const outputPath = path.join(tmpDir, `opus_out_${uid}.ogg`);
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            ...
            proc.on('close', code => {
                ...
                else reject(new Error('convertToOpusBuffer ffmpeg failed: ' + stderr.slice(-300)));
            });
        });
...
const uploadWithNativeHttps = (url, body, headers, agent, timeoutMs, redirectCount = 0) => {
    if (redirectCount > 5) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const req = https.request({
            ...
            if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                ...
                resolve(uploadWithNativeHttps(newUrl, body, headers, agent, timeoutMs, redirectCount + 1));
                return;
            }
            let responseBody = '';
    ...
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        for (const { hostname, maxContentLengthBytes } of hosts) {
            ...
            try {
                ...
                result = await uploadWithNativeHttps(url, reqBody, headers, fetchAgent, timeoutMs);
                ...
                const isLast = hostname === (hosts[hosts.length - 1] === null || hosts[hosts.length - 1] === void 0 ? void 0 : hosts[hosts.length - 1].hostname);
